            ...
            <Button size={'sm'} variant={'link'} onClick={() => { setOpen(!open) }} >Reply</Button>
            {
                open && (
                    <form action={action} className='space-y-2'>
                        ...
                        <Textarea name='content' placeholder='write a comment...' className='bg-gray-100 focus-visible:ring-0' />
                        
                           { formState.errors.content && <p className='text-red-600 text-sm'>{formState.errors.content}</p>}
                          {  formState.errors.formError && <div className='bg-red-200 border border-red-600 text-sm p-2 rounded'>{formState.errors.formError}</div>}
